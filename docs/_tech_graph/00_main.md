# 顶层流程总图（cyning-harness-web · A0）

> **用途**：本仓 Demo 主路径（人类可读简版）。  
> **Phase**：A0 Inform · 手写 flowchart；后续可迁 YAML-first（见 `99_mermaid_protocol.md`）。  
> **边界**：落盘真值在 `docs/**`；Web 只读投影，不写闸。

## 主路径一句话

**Agent 落盘**（task / invoke / review / evidence）→ **obs_api 读盘 / spawn CLI** → **Web 只读投影**（`/` · `/obs` · `/docs`）。
...
- 本图 **不**展开 Phase A 脚手架实现细节（属后续 `scaffold_*` task）
