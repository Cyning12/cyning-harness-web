# 顶层流程总图（cyning-harness-web · A0→D）

> **用途**：本仓 Demo 主路径（人类可读简版）。  
> **Phase**：A0 Inform + A 脚手架 + B live CLI + C dogfood + **D HGM 消费者**；后续可迁 YAML-first（见 `99_mermaid_protocol.md`）。  
...
> **graph_delta（Phase D）**：模块边界未变；`obs_api` 消费 timeline 时 **默认不 ingest**；仅显式 `?ingest=1` / UI 勾选才追加 CLI `--ingest`（会写 events）；空事件 / WARN 可读投影。
