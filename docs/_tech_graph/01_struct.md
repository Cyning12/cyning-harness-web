...
| `obs_api` | 薄观测 API | `server/**` | → `harness_docs` · `evidence` | Vite middleware：读盘 `docs/**` md（列表默认 `docs/tasks`）+ live `status`/`timeline --json`（可 stub）+ **只读** `GET /api/obs/harness-version`（pin / manifest / npm latest）；正文相对 `.md` 链接按本地语义跳转；CLI 包规格读仓根 `harness.pin.json`；**禁止**写闸 / 触发 `upgrade` / **默认不** ingest；仅显式 `?ingest=1` 才写 events |
