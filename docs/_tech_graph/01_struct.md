...
|-----------|------|-----------|----------------------|-------------|
| `web_ui` | Web 只读 UI | `src/**` | → `obs_api` | Vite/Vue 页面与路由（`/` · `/obs` · `/docs` · `/wiki-graph`）；Wiki 图为只读力导向投影 |
| `obs_api` | 薄观测 API | `server/**` | → `harness_docs` · `evidence` | Vite middleware：读盘 `docs/**` md + live `status`/`timeline --json` + `harness-version` + **`GET /api/obs/wiki-graph`**（`wiki export --json`，默认 root `docs/coding_wiki`）；CLI 读 `harness.pin.json`；**禁止**写闸 / 浏览器 npx / 默认不 ingest |
| `harness_docs` | Harness 落盘真值 | `docs/tasks/**` · `docs/harness/**` · `docs/spec/**` · `docs/coding_wiki/**` | —（被依赖） | task / invoke / review / auth / coding_wiki；签收真值所在 |
